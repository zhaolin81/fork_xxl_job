...
import com.xxl.job.admin.core.model.XxlJobResultInfo;
...
public class XxlJobServiceImpl implements IXxlJobService {
	...
	@Override
	public ReturnT<String> pauseAll(String jobGroup) {
		List<XxlJobInfo> xxlJobInfoList = xxlJobInfoDao.getJobsByGroup(jobGroup);
		if(xxlJobInfoList != null && (!xxlJobInfoList.isEmpty())){
			Set<XxlJobResultInfo> results = new HashSet<XxlJobResultInfo>();
			for(XxlJobInfo xxlJobInfo:xxlJobInfoList){
				String group = String.valueOf(xxlJobInfo.getJobGroup());
				String name = String.valueOf(xxlJobInfo.getId());
				try {
					boolean ret = XxlJobDynamicScheduler.pauseJob(name, group);	// jobStatus do not store
					results.add(XxlJobResultInfo.buildResultInfo(xxlJobInfo,ret));
				} catch (SchedulerException e) {
					e.printStackTrace();
					results.add(XxlJobResultInfo.buildResultInfo(xxlJobInfo,false));
				}
			}
			return new ReturnT(results);
		}
		return ReturnT.SUCCESS;
	}
	...
	@Override
	public ReturnT<String> resumeAll(String jobGroup) {
		List<XxlJobInfo> xxlJobInfoList = xxlJobInfoDao.getJobsByGroup(jobGroup);
		if(xxlJobInfoList != null && (!xxlJobInfoList.isEmpty())){
			Set<XxlJobResultInfo> results = new HashSet<XxlJobResultInfo>();
			for(XxlJobInfo xxlJobInfo:xxlJobInfoList){
				...
				String name = String.valueOf(xxlJobInfo.getId());
				try {
					boolean ret = XxlJobDynamicScheduler.resumeJob(name, group);	// jobStatus do not store
					results.add(XxlJobResultInfo.buildResultInfo(xxlJobInfo,ret));
				} catch (SchedulerException e) {
					e.printStackTrace();
					results.add(XxlJobResultInfo.buildResultInfo(xxlJobInfo,false));
