...
public class PermissionInterceptor extends HandlerInterceptorAdapter {
    ...
    //api accesstoken
	public static final String ACCESS_TOKEN = PropertiesUtil.getString("xxl.job.access.token");
	...
	public static boolean ifLogin(HttpServletRequest request){
		...
		if (indentityInfo==null || !LOGIN_IDENTITY_TOKEN.equals(indentityInfo.trim())) {
			//没有会话，尝试 accessToken 访问方式
			String accessToken = request.getParameter(ACCESS_TOKEN_KEY);
			if (accessToken==null || !ACCESS_TOKEN.equals(accessToken.trim())) {
				return false;
			}
		}
		return true;
